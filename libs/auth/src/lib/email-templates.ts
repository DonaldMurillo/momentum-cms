 ...
 */
function wrapEmail(content: string, appName: string): string {
	return `
<!DOCTYPE html>
<html lang="en">
<head>
...
`.trim();
	...
	const html = wrapEmail(
      ...
      <p style="margin: 0 0 16px; color: #3f3f46;">${greeting}</p>
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td style="padding: 0 0 24px;">
            <a href="${url}" style="display: inline-block; padding: 12px 24px; background-color: #18181b; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 500;">Reset Password</a>
          </td>
        </tr>
      </table>
      <p style="margin: 0 0 8px; color: #71717a; font-size: 14px;">This link will expire in ${expiresIn}.</p>
      ...
      <p style="margin: 8px 0 0; color: #71717a; font-size: 12px; word-break: break-all;">${url}</p>
    `,
		appName,
	);
...
`.trim();
	...
	const html = wrapEmail(
      ...
      <p style="margin: 0 0 16px; color: #3f3f46;">${greeting}</p>
      <p style="margin: 0 0 24px; color: #3f3f46;">Welcome to ${appName}! Please verify your email address by clicking the button below:</p>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td style="padding: 0 0 24px;">
            <a href="${url}" style="display: inline-block; padding: 12px 24px; background-color: #18181b; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 500;">Verify Email</a>
          </td>
        </tr>
      </table>
      <p style="margin: 0 0 8px; color: #71717a; font-size: 14px;">This link will expire in ${expiresIn}.</p>
