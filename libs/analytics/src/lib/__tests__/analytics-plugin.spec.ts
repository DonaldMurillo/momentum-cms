...
import type { PluginContext, PluginMiddlewareDescriptor } from '@momentum-cms/plugins';
...
describe('analyticsPlugin', () => {
	...
	it('should register ingest router and API collector middleware during onInit', async () => {
		...
		// Second: API collector
		expect(registeredMiddleware[1].path).toBe('/');
	...
	it('should skip API collector when trackApi is false', async () => {
		...
		// Only ingest router should be registered (not API collector)
