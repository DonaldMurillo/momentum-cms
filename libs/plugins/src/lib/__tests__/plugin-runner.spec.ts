...
describe('PluginRunner', () => {
	...
	it('should return empty arrays when no plugins register middleware/providers/admin routes', async () => {
		...
		expect(runner.getAdminRoutes()).toEqual([]);
	});

	it('should collect admin routes registered during onInit', async () => {
		const loadFn = (): Promise<unknown> => Promise.resolve({ TestComponent: {} });
		const plugins: MomentumPlugin[] = [
			{
				name: 'ui-plugin',
				onInit: (ctx) => {
		...
		const routes = runner.getAdminRoutes();
		expect(routes).toHaveLength(1);
		expect(routes[0].path).toBe('analytics');
		expect(routes[0].label).toBe('Analytics');
		expect(routes[0].icon).toBe('heroChartBarSquare');
		expect(routes[0].loadComponent).toBe(loadFn);
		expect(routes[0].group).toBe('Tools');
	});

	it('should preserve admin route registration order across plugins', async () => {
		const plugins: MomentumPlugin[] = [
			{
				name: 'first',
				onInit: (ctx) => {
		...
		expect(routes.map((r) => r.path)).toEqual(['analytics', 'logs']);
	});
