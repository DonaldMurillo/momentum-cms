...
test.describe(
	'Observability Plugin — Prometheus Metrics',
	{ tag: ['@otel', '@api'] },
	() => {
...
test.describe(
	'Observability Plugin — Admin Dashboard',
	{ tag: ['@otel', '@admin'] },
	() => {
		test('As an admin, I should be able to see my system health from the observability dashboard', async ({
			...
			await expect(
				authenticatedPage.getByRole('heading', { name: /Observability/i }),
			).toBeVisible({ timeout: 15000 });

			// System Health section should show uptime (proves real data loaded)
			await expect(
				authenticatedPage.getByRole('heading', { name: 'System Health' }),
			).toBeVisible({ timeout: 15000 });

			// Request Metrics section should exist (page load itself generates requests)
			await expect(
				authenticatedPage.getByRole('heading', { name: 'Request Metrics' }),
			).toBeVisible({ timeout: 15000 });
		});
		...
		test('As an admin, after creating content I should see that activity in the observability dashboard', async ({
			...
			try {
				...
				await expect(
					authenticatedPage.getByText('Collection Operations'),
				).toBeVisible({ timeout: 15000 });
...
test.describe(
	'Observability Plugin — Security Boundaries',
	{ tag: ['@otel', '@security'] },
	() => {
		...
		test('As an admin, I should be able to access the summary API', async ({
			request,
		}) => {
			// Sign in as admin
			const signIn = await request.post('/api/auth/sign-in/email', {
