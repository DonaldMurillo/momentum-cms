...
test.describe('Authentication Flow', { tag: ['@auth', '@admin'] }, () => {
	test.describe('Setup Page', () => {
		...
		// Setup-form validation tests (empty/short/mismatched password) used to
		// live here, but they all skipped once the worker fixture seeded an
		// admin user (because /admin/setup then redirects to /login). The
		// equivalent assertions are covered deterministically by the
		// SetupPage component unit tests in
		// libs/admin/src/lib/pages/setup/__tests__/setup.page.spec.ts —
		// keeping them as flaky, always-skipped e2e duplicates added noise
