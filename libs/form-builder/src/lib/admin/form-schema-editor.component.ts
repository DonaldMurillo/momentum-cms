 ...
 *
 * Draggable field list with card-based editing.
 * Live preview is handled by the side panel via LivePreviewService.
 */
@Component({
	...
	host: { class: 'block' },
	template: `
		<div class="p-4" data-testid="form-schema-editor">
			<!-- Form Settings -->
				...
				>
					@for (field of editorState.fields(); track field.name; let i = $index) {
						...
						>
							...
							@if (field.required) {
								<span class="text-xs text-destructive shrink-0" aria-label="Required field">*</span>
							}
