...
| **Server adapters**    | Phases 1-5 of the consolidation are shipped — Access, Status, Globals, Versioning, Publishing, and Batch are now thin dispatchers around shared `server-core` handlers in all three adapters. Express is down to ~1,125 lines and Analog to ~1,251. NestJS DI controllers exist for Access, Globals, Status, Version, and Publishing routes; **still missing**: optional NestJS controllers for Media, Upload, Batch, Search, ImportExport, Preview, GraphQL, and CustomEndpoints. Express/Analog still inline a handful of niche routes (preview, GraphQL, file upload, custom endpoints) that haven't been extracted yet. | Default `createMomentumNestServer` already has full route parity via Express middleware, so users see no functional gap. The remaining work is purely about ergonomics for users wiring their own NestJS modules and removing the last duplicated logic from Express/Analog. |
