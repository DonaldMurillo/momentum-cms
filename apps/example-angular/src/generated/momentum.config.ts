/**
 * AUTO-GENERATED by @momentumcms/core
 * DO NOT EDIT - regenerate with: nx run <app>:generate
 */

import type { MomentumAdminConfig } from '@momentumcms/core';
import type { CollectionSlug, GlobalSlug } from './momentum.types';
import { analyticsAdminRoutes } from '@momentumcms/plugins/analytics/admin-routes';

export const adminConfig: MomentumAdminConfig<CollectionSlug, GlobalSlug> = {
	collections: [
		{
			slug: "categories",
			labels: {
				singular: "Category",
				plural: "Categories",
			},
			fields: [
				{
					name: "name",
					type: "text",
					required: true,
					label: "Name",
				},
				{
					name: "slug",
					type: "text",
					required: true,
					label: "Slug",
				},
			],
			admin: {
				group: "Content",
			},
			timestamps: true,
		},
		{
			slug: "articles",
			labels: {
				singular: "Article",
				plural: "Articles",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
					label: "Title",
				},
				{
					name: "content",
					type: "richText",
					label: "Content",
				},
				{
					name: "category",
					type: "relationship",
					label: "Category",
					collection: () => ({ slug: "categories", labels: { singular: "Category", plural: "Categories" } }),
				},
			],
			admin: {
				group: "Content",
			},
			timestamps: true,
			versions: {
				drafts: true,
				maxPerDoc: 10,
			},
		},
		{
			slug: "products",
			labels: {
				singular: "Product",
				plural: "Products",
			},
			fields: [
				{
					name: "name",
					type: "text",
					required: true,
					label: "Product Name",
				},
				{
					name: "description",
					type: "textarea",
					label: "Description",
				},
				{
					name: "price",
					type: "number",
					label: "Price",
					min: 0,
					displayFormat: {
							style: "currency",
							currency: "USD",
							minimumFractionDigits: 2,
						},
				},
				{
					name: "seo",
					type: "group",
					label: "SEO Settings",
					description: "Search engine optimization settings for this product.",
					fields: [
							{
								name: "metaTitle",
								type: "text",
								label: "Meta Title",
							},
							{
								name: "metaDescription",
								type: "textarea",
								label: "Meta Description",
							},
							{
								name: "ogImage",
								type: "text",
								label: "OG Image URL",
							},
						],
				},
				{
					name: "features",
					type: "array",
					label: "Features",
					description: "Product features list.",
					fields: [
							{
								name: "label",
								type: "text",
								required: true,
								label: "Feature Label",
							},
							{
								name: "description",
								type: "text",
								label: "Feature Description",
							},
							{
								name: "highlighted",
								type: "checkbox",
								label: "Highlighted",
								defaultValue: false,
							},
						],
					minRows: 0,
					maxRows: 10,
				},
			],
			timestamps: true,
		},
		{
			slug: "pages",
			labels: {
				singular: "Page",
				plural: "Pages",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
					label: "Page Title",
				},
				{
					name: "slug",
					type: "text",
					required: true,
					label: "URL Slug",
				},
				{
					name: "content",
					type: "blocks",
					label: "Page Content",
					description: "Build your page using content blocks.",
					admin: {
							editor: "visual",
						},
					blocks: [
							{
								slug: "hero",
								fields: [
									{
										name: "heading",
										type: "text",
										required: true,
										label: "Heading",
									},
									{
										name: "subheading",
										type: "textarea",
										label: "Subheading",
									},
									{
										name: "ctaText",
										type: "text",
										label: "CTA Button Text",
									},
									{
										name: "ctaLink",
										type: "text",
										label: "CTA Button Link",
									},
									{
										name: "_analytics",
										type: "group",
										label: "Analytics",
										admin: {
												collapsible: true,
												defaultOpen: false,
											},
										fields: [
												{
													name: "trackImpressions",
													type: "checkbox",
													label: "Track Impressions",
													defaultValue: false,
												},
												{
													name: "trackHover",
													type: "checkbox",
													label: "Track Hover",
													defaultValue: false,
												},
											],
									},
								],
								labels: {
									singular: "Hero",
									plural: "Heroes",
								},
							},
							{
								slug: "textBlock",
								fields: [
									{
										name: "heading",
										type: "text",
										label: "Section Heading",
									},
									{
										name: "body",
										type: "textarea",
										required: true,
										label: "Body Text",
									},
									{
										name: "_analytics",
										type: "group",
										label: "Analytics",
										admin: {
												collapsible: true,
												defaultOpen: false,
											},
										fields: [
												{
													name: "trackImpressions",
													type: "checkbox",
													label: "Track Impressions",
													defaultValue: false,
												},
												{
													name: "trackHover",
													type: "checkbox",
													label: "Track Hover",
													defaultValue: false,
												},
											],
									},
								],
								labels: {
									singular: "Text Block",
									plural: "Text Blocks",
								},
							},
							{
								slug: "feature",
								fields: [
									{
										name: "title",
										type: "text",
										required: true,
										label: "Feature Title",
									},
									{
										name: "description",
										type: "textarea",
										label: "Feature Description",
									},
									{
										name: "icon",
										type: "text",
										label: "Icon Name",
									},
									{
										name: "_analytics",
										type: "group",
										label: "Analytics",
										admin: {
												collapsible: true,
												defaultOpen: false,
											},
										fields: [
												{
													name: "trackImpressions",
													type: "checkbox",
													label: "Track Impressions",
													defaultValue: false,
												},
												{
													name: "trackHover",
													type: "checkbox",
													label: "Track Hover",
													defaultValue: false,
												},
											],
									},
								],
								labels: {
									singular: "Feature",
									plural: "Features",
								},
							},
						],
				},
			],
			admin: {
				group: "Content",
			},
			timestamps: true,
		},
		{
			slug: "settings",
			labels: {
				singular: "Setting",
				plural: "Settings",
			},
			fields: [
				{
					name: "settingsTabs",
					type: "tabs",
					label: "Site Settings",
					tabs: [
							{
								label: "General",
								fields: [
									{
										name: "siteName",
										type: "text",
										required: true,
										label: "Site Name",
									},
									{
										name: "siteDescription",
										type: "textarea",
										label: "Site Description",
									},
								],
							},
							{
								label: "Social",
								fields: [
									{
										name: "socialRow",
										type: "row",
										label: "Social Profiles",
										fields: [
												{
													name: "twitterHandle",
													type: "text",
													label: "Twitter Handle",
												},
												{
													name: "facebookUrl",
													type: "text",
													label: "Facebook URL",
												},
											],
									},
									{
										name: "linkedinUrl",
										type: "text",
										label: "LinkedIn URL",
									},
								],
							},
							{
								name: "notifications",
								label: "Notifications",
								description: "Configure notification preferences.",
								fields: [
									{
										name: "emailEnabled",
										type: "checkbox",
										label: "Email Notifications Enabled",
										defaultValue: false,
									},
									{
										name: "emailFrom",
										type: "text",
										label: "Sender Email Address",
									},
								],
							},
						],
				},
				{
					name: "advanced",
					type: "collapsible",
					label: "Advanced Settings",
					description: "These settings are for advanced users.",
					fields: [
							{
								name: "analyticsId",
								type: "text",
								label: "Analytics ID",
							},
							{
								name: "maintenanceMode",
								type: "checkbox",
								label: "Maintenance Mode",
								defaultValue: false,
							},
						],
				},
			],
			timestamps: true,
		},
		{
			slug: "events",
			labels: {
				singular: "Event",
				plural: "Events",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
					label: "Title",
				},
				{
					name: "description",
					type: "textarea",
					label: "Description",
				},
				{
					name: "location",
					type: "text",
					label: "Location",
				},
				{
					name: "eventDate",
					type: "date",
					label: "Event Date",
				},
			],
			admin: {
				useAsTitle: "title",
				preview: true,
			},
			timestamps: true,
		},
		{
			slug: "media",
			labels: {
				singular: "Media",
				plural: "Media",
			},
			fields: [
				{
					name: "filename",
					type: "text",
					required: true,
					label: "Filename",
					description: "Original filename of the uploaded file",
				},
				{
					name: "mimeType",
					type: "text",
					required: true,
					label: "MIME Type",
					description: "File MIME type (e.g., image/jpeg, application/pdf)",
				},
				{
					name: "filesize",
					type: "number",
					label: "File Size",
					description: "File size in bytes",
				},
				{
					name: "path",
					type: "text",
					required: true,
					label: "Storage Path",
					description: "Path/key where the file is stored",
					admin: {
							hidden: true,
						},
				},
				{
					name: "url",
					type: "text",
					label: "URL",
					description: "Public URL to access the file",
				},
				{
					name: "alt",
					type: "text",
					label: "Alt Text",
					description: "Alternative text for accessibility",
				},
				{
					name: "width",
					type: "number",
					label: "Width",
					description: "Image width in pixels (for images only)",
				},
				{
					name: "height",
					type: "number",
					label: "Height",
					description: "Image height in pixels (for images only)",
				},
				{
					name: "focalPoint",
					type: "json",
					label: "Focal Point",
					description: "Focal point coordinates for image cropping",
					admin: {
							hidden: true,
						},
				},
			],
			admin: {
				useAsTitle: "filename",
				defaultColumns: [
					"filename",
					"mimeType",
					"filesize",
					"createdAt",
				],
			},
			timestamps: true,
		},
		{
			slug: "hook-test-items",
			labels: {
				singular: "HookTestItem",
				plural: "HookTestItems",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
					label: "Title",
				},
				{
					name: "slug",
					type: "text",
					label: "Slug",
				},
				{
					name: "status",
					type: "text",
					label: "Status",
				},
				{
					name: "priority",
					type: "number",
					label: "Priority",
				},
			],
			timestamps: true,
		},
		{
			slug: "field-test-items",
			labels: {
				singular: "Field Test Item",
				plural: "Field Test Items",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
					minLength: 3,
					maxLength: 100,
				},
				{
					name: "code",
					type: "text",
					required: true,
					minLength: 2,
					maxLength: 10,
				},
				{
					name: "contactEmail",
					type: "email",
				},
				{
					name: "rating",
					type: "number",
					min: 1,
					max: 5,
					step: 1,
				},
				{
					name: "price",
					type: "number",
					min: 0,
				},
				{
					name: "status",
					type: "select",
					options: [
							{
								label: "Active",
								value: "active",
							},
							{
								label: "Draft",
								value: "draft",
							},
							{
								label: "Archived",
								value: "archived",
							},
						],
					required: true,
				},
				{
					name: "tags",
					type: "array",
					fields: [
							{
								name: "label",
								type: "text",
								required: true,
							},
						],
					minRows: 1,
					maxRows: 5,
				},
				{
					name: "adminNotes",
					type: "text",
				},
				{
					name: "internalCode",
					type: "text",
				},
				{
					name: "internalScore",
					type: "number",
				},
				{
					name: "slug",
					type: "text",
				},
				{
					name: "normalizedTitle",
					type: "text",
				},
				{
					name: "viewCount",
					type: "number",
				},
			],
			timestamps: true,
		},
		{
			slug: "tags",
			labels: {
				singular: "Tag",
				plural: "Tags",
			},
			fields: [
				{
					name: "name",
					type: "text",
					required: true,
					label: "Name",
				},
				{
					name: "category",
					type: "relationship",
					label: "Category",
					collection: () => ({ slug: "categories", labels: { singular: "Category", plural: "Categories" } }),
					required: true,
				},
			],
			timestamps: true,
		},
		{
			slug: "user-notes",
			labels: {
				singular: "User Note",
				plural: "User Notes",
			},
			fields: [
				{
					name: "title",
					type: "text",
					required: true,
				},
				{
					name: "ownerId",
					type: "text",
					admin: {
							hidden: true,
						},
				},
			],
			timestamps: true,
		},
		{
			slug: "auth-user",
			labels: {
				singular: "User",
				plural: "Users",
			},
			fields: [
				{
					name: "name",
					type: "text",
					required: true,
				},
				{
					name: "email",
					type: "email",
					required: true,
				},
				{
					name: "emailVerified",
					type: "checkbox",
					defaultValue: false,
				},
				{
					name: "image",
					type: "text",
				},
				{
					name: "role",
					type: "select",
					options: [
							{
								label: "Admin",
								value: "admin",
							},
							{
								label: "Editor",
								value: "editor",
							},
							{
								label: "User",
								value: "user",
							},
							{
								label: "Viewer",
								value: "viewer",
							},
						],
					defaultValue: "user",
				},
			],
			admin: {
				group: "Authentication",
				useAsTitle: "email",
				defaultColumns: [
					"name",
					"email",
					"role",
					"createdAt",
				],
				description: "Users authenticated via Better Auth",
			},
			timestamps: true,
		},
		{
			slug: "auth-session",
			fields: [
				{
					name: "userId",
					type: "text",
					required: true,
				},
				{
					name: "token",
					type: "text",
					required: true,
				},
				{
					name: "expiresAt",
					type: "date",
					required: true,
				},
				{
					name: "ipAddress",
					type: "text",
				},
				{
					name: "userAgent",
					type: "text",
				},
			],
			admin: {
				group: "Authentication",
				hidden: true,
				description: "Active user sessions",
			},
			timestamps: true,
			managed: true,
		},
		{
			slug: "auth-account",
			fields: [
				{
					name: "userId",
					type: "text",
					required: true,
				},
				{
					name: "accountId",
					type: "text",
					required: true,
				},
				{
					name: "providerId",
					type: "text",
					required: true,
				},
				{
					name: "accessToken",
					type: "text",
				},
				{
					name: "refreshToken",
					type: "text",
				},
				{
					name: "accessTokenExpiresAt",
					type: "date",
				},
				{
					name: "refreshTokenExpiresAt",
					type: "date",
				},
				{
					name: "scope",
					type: "text",
				},
				{
					name: "idToken",
					type: "text",
				},
				{
					name: "password",
					type: "text",
				},
			],
			admin: {
				group: "Authentication",
				hidden: true,
				description: "OAuth and credential accounts",
			},
			timestamps: true,
			managed: true,
		},
		{
			slug: "auth-verification",
			fields: [
				{
					name: "identifier",
					type: "text",
					required: true,
				},
				{
					name: "value",
					type: "text",
					required: true,
				},
				{
					name: "expiresAt",
					type: "date",
					required: true,
				},
			],
			admin: {
				group: "Authentication",
				hidden: true,
				description: "Email verification and password reset tokens",
			},
			timestamps: true,
			managed: true,
		},
		{
			slug: "auth-api-keys",
			fields: [
				{
					name: "name",
					type: "text",
					required: true,
				},
				{
					name: "keyHash",
					type: "text",
					required: true,
					admin: {
							hidden: true,
						},
				},
				{
					name: "keyPrefix",
					type: "text",
					required: true,
				},
				{
					name: "createdBy",
					type: "relationship",
					required: true,
					collection: () => ({ slug: "auth-user", labels: { singular: "User", plural: "Users" }, admin: { useAsTitle: "email" } }),
					label: "Created By",
				},
				{
					name: "role",
					type: "select",
					options: [
							{
								label: "Admin",
								value: "admin",
							},
							{
								label: "Editor",
								value: "editor",
							},
							{
								label: "User",
								value: "user",
							},
							{
								label: "Viewer",
								value: "viewer",
							},
						],
					defaultValue: "user",
				},
				{
					name: "expiresAt",
					type: "date",
				},
				{
					name: "lastUsedAt",
					type: "date",
				},
			],
			admin: {
				group: "Authentication",
				useAsTitle: "name",
				defaultColumns: [
					"name",
					"keyPrefix",
					"role",
					"createdBy",
					"createdAt",
					"lastUsedAt",
				],
				description: "API keys for programmatic access",
				headerActions: [
					{
						id: "generate-key",
						label: "Generate API Key",
						endpoint: "/api/auth/api-keys",
					},
				],
			},
			timestamps: true,
		},
	],
	globals: [
		{
			slug: "site-settings",
			label: "Site Settings",
			fields: [
				{
					name: "site-name",
					type: "text",
					required: true,
					label: "Site Name",
				},
				{
					name: "description",
					type: "textarea",
					label: "Site Description",
				},
				{
					name: "maintenance-mode",
					type: "checkbox",
					label: "Maintenance Mode",
					defaultValue: false,
				},
			],
		},
	],
	admin: {
		basePath: "/admin",
		branding: {
			title: "Momentum CMS",
		},
		toasts: true,
	},
	plugins: [
		{ name: "analytics", adminRoutes: analyticsAdminRoutes },
	],
};
